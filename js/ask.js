...
function loadPosts() {
  ...
  loadingPosts = true;
  let rUrl = 'https://ask.3schools.in/feeds/posts/default?alt=json&max-results=' + maxResults + '&q=' + rK + '&callback=qnaR';
  let rScript = document.createElement('script');
  rScript.src = rUrl;

  window.qnaR = function(response) {
    let entries = response.feed.entry;

    let postContainer = document.querySelector('.recommend');
    for (let i = 0; i < entries.length; i++) {
      let c = entries[i];
      let postTitle = c.title.$t;

      if (loadedPosts.indexOf(postTitle) === -1) {
...
<div class="flex3s">
<div class="auth-img">
<img alt="${c.author[0].name.$t}" class="lazyload" data-src="${c.author[0].gd$image.src}">
</div>
<div class="auth-name">
<small>${c.author[0].name.$t} <svg fill='#00acee' viewBox='0 0 24 24'><path d='M23,12L20.56,9.22L20.9,5.54L17.29,4.72L15.4,1.54L12,3L8.6,1.54L6.71,4.72L3.1,5.53L3.44,9.21L1,12L3.44,14.78L3.1,18.47L6.71,19.29L8.6,22.47L12,21L15.4,22.46L17.29,19.28L20.9,18.46L20.56,14.78L23,12M10,17L6,13L7.41,11.59L10,14.17L16.59,7.58L18,9L10,17Z'></path></svg>
</small>
<b>Related</b>
</div>
</div>
<div onclick="feedBack(this)" id="feedback" class="${c.title.$t}_${c.link[c.link.length-1].href}_${document.title}">❌</div>
</div>
<div class="gradient">
<h2 class="post-title">
<a data-text="${c.title.$t}" href="${c.link[c.link.length-1].href}" rel="bookmark">${c.title.$t}</a>
</h2>
<p class="post-des" data-text="${c.content.$t.replace(/<[^>]*>?/g, '').substring(0, 150) + '...'}"></p>
</div>
<div class="flex3s post-share m">
<a href="${c.link[c.link.length-1].href}#comments"><svg class='line' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'><polyline points='15 14 20 9 15 4'/><path d='M4 20v-7a4 4 0 0 1 4-4h12'/></svg> উত্তর দিন</a>
<div data-title="${c.title.$t}" data-url="${c.link[c.link.length-1].href}" onclick="share(this)"><svg class="line" viewBox="0 0 24 24"><path d="M92.30583,264.72053a3.42745,3.42745,0,0,1-.37,1.57,3.51,3.51,0,1,1,0-3.13995A3.42751,3.42751,0,0,1,92.30583,264.72053Z" transform="translate(-83.28571 -252.73452)"></path><circle class="fill" cx="18.48892" cy="5.49436" r="3.51099"></circle><circle class="fill" cx="18.48892" cy="18.50564" r="3.51099"></circle><line class="cls-3" x1="12.53012" x2="8.65012" y1="8.476" y2="10.416"></line><line class="cls-3" x1="12.53012" x2="8.65012" y1="15.496" y2="13.556"></line></svg> শেয়ার</div>
<div data-des="${c.content.$t.replace(/<[^>]*>?/g, '').substring(0, 150) + '...'}" data-title="${c.title.$t}" data-url="${c.link[c.link.length-1].href}" onclick="addItem(this)"><svg preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M17 3H7c-1.1 0-1.99.9-1.99 2L5 21l7-3l7 3V5c0-1.1-.9-2-2-2z" fill="currentColor"></path></svg> সেভ</div>
<div onclick="speak(this.parentElement.previousElementSibling.innerText +''+ this.parentElement.previousElementSibling.querySelector('.post-des').dataset.text)"><svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M13.5 4.06c0-1.336-1.616-2.005-2.56-1.06l-4.5 4.5H4.508c-1.141 0-2.318.664-2.66 1.905A9.76 9.76 0 001.5 12c0 .898.121 1.768.35 2.595.341 1.24 1.518 1.905 2.659 1.905h1.93l4.5 4.5c.945.945 2.561.276 2.561-1.06V4.06zM18.584 5.106a.75.75 0 011.06 0c3.808 3.807 3.808 9.98 0 13.788a.75.75 0 11-1.06-1.06 8.25 8.25 0 000-11.668.75.75 0 010-1.06z"></path><path d="M15.932 7.757a.75.75 0 011.061 0 6 6 0 010 8.486.75.75 0 01-1.06-1.061 4.5 4.5 0 000-6.364.75.75 0 010-1.06z"></path></svg> শুনুন</div>
</div>
        `;
        
      postContainer.appendChild(postElement);
       loadedPosts.push(postTitle);
      if (loadedPosts.length >= maxResults) {
          loadingPosts = false;
          break;
        }
      if(i%5 === 0 && i != 0 && radsC.length > 0){
        let rAds = document.createElement('article');rAds.innerHTML=radsC[Math.floor(Math.random() * radsC.length)];postElement.insertAdjacentElement('afterend', rAds)
      }
      }
    }
    loadingPosts = false;
...
let loadNowContainer = document.getElementById('load-now');
window.addEventListener('scroll', function() {
  let containerTop = loadNowContainer.getBoundingClientRect().top;
  let windowHeight = window.innerHeight;
  if (containerTop - windowHeight <= 0 && !loadingPosts) {
    maxResults += 5;
    loadPosts();
  }
});
