...
var feedback = function(res) {
    ...
    if (res.success === true) {
        ...
        var content = `<br><input class="image-url" value=\"` + get_link + `\"/><button onclick="var t = document.querySelector('.image-url');t.select();document.execCommand('copy');">copy</button> <input class="image-urls" value='<img src=\"` + get_link + `\"/>'/><button onclick="var t = document.querySelector('.image-urls');t.select();document.execCommand('copy');">copy</button><br> <img class="img" alt="Imgur-Upload" src=\"` + get_link + `\"/>`;
        addImg('.status', content);
    }
};

new Imgur({
    clientid: 'a9fa50cf97cf7fa', //You can change this ClientID
    callback: feedback
});
